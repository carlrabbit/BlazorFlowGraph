...
The older `tooling/scripts/start-samples-all-background.sh` wrapper remains only so existing callers do not break; direct `--detach` mode is the supported path.
